...
function initFormulaLibrary() {
    ...
    const formulaLibrary = document.querySelector('.formula-library');
    ...
    categorySelect.addEventListener('change', (e) => {
        ...
        displayFormulas(selectedCategory);

        // Adjust layout
        if (selectedCategory !== 'Basic Math') {
            calculatorWrapper.style.marginRight = '20rem';
            formulaLibrary.style.top = '40px';
            formulaLibrary.style.right = '20px';
            quickNotes.style.right = '350px';
            quickNotes.style.top = '10px';
        } else {
            calculatorWrapper.style.marginRight = '10rem';
            formulaLibrary.style.top = '230px';
            formulaLibrary.style.right = '18px';
            quickNotes.style.right = '20px';
            quickNotes.style.top = 'auto';
        }
    });
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Function to adjust layout when a category is selected
        function adjustLayoutForFormulas() {
            // Move calculator wrapper
            calculatorWrapper.style.marginRight = '20rem';
            
            // Adjust formula library position
            formulaLibrary.style.top = '40px';
            formulaLibrary.style.right = '20px';
            
            // Adjust quick notes
            quickNotes.style.position = 'absolute';
            quickNotes.style.right = '350px';
            quickNotes.style.top = '10px';
        }
        ...
        categorySelect.addEventListener('change', (e) => {
            ...
            // Only adjust layout if a real category is selected (not the default)
            if (selectedCategory !== 'Choose a Category') {
                adjustLayoutForFormulas();
            } else {
                // Reset to default layout if default option is selected
                calculatorWrapper.style.marginRight = '10rem';
                formulaLibrary.style.top = '230px';
                formulaLibrary.style.right = '18px';
                quickNotes.style.right = '20px';
        ...
        // Setup use formula button event delegation
        formulaLibrary.addEventListener('click', (e) => {
            if (e.target.classList.contains('use-formula')) {
                const formulaItem = e.target.closest('.formula-item');
                if (formulaItem) {
                    const formulaText = formulaItem.querySelector('.formula-text');
                    if (formulaText) {
                        // Extract the formula part (everything after the ':')
                        const fullText = formulaText.textContent;
                        const formula = fullText.split(':')[1].trim();
        ...
        // Optional: Prevent default "Choose a Category" from being useful
        categorySelect.querySelector('option[value="basic-math"]').textContent = 'Choose a Category';
    });
    
    function displayFormulas(category) {
        const formulaDisplay = document.getElementById('formula-display');
        formulaDisplay.innerHTML = ''; // Clear previous formulas
    
        // Check if the category exists in the formulas object
        if (!formulas[category]) {
            console.error('Category not found:', category);
            return;
        }
    
        // Create formulas for the selected category
        for (let formulaName in formulas[category]) {
            const formulaDiv = document.createElement('div');
            formulaDiv.className = 'formula-item';
            
            const formulaText = document.createElement('div');
            formulaText.className = 'formula-text';
            formulaText.textContent = `${formulaName}: ${formulas[category][formulaName]}`;
